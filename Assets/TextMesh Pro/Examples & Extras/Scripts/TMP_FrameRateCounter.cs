...
namespace TMPro.Examples
{
    ...
    public class TMP_FrameRateCounter : MonoBehaviour
    {
        ...
        private const string fpsLabel = "{0:2}</currentColor> <#8080ff>FPS \n<#FF8000>{1:2} <#8080ff>MS";
        ...
        void Update()
        {
            ...
            if (timeNow > m_LastInterval + UpdateInterval)
            {
                ...
                if (fps < 30)
                    htmlColorTag = "<currentColor=yellow>";
                else if (fps < 10)
                    htmlColorTag = "<currentColor=red>";
                else
                    htmlColorTag = "<currentColor=green>";

                //string format = System.String.Format(htmlColorTag + "{0:F2} </currentColor>FPS \n{1:F2} <#8080ff>MS",fps, ms);
